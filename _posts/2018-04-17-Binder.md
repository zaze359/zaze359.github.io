...
3. init程序负责解析init.rc配置文件执行Action和Service,开启系统守护进程。其中最重要的是**zygote**和**ServiceManager**。前者是Android启动的第一个Dalvik虚拟机(4.4以上应该是ART吧)，它负责启动java世界的进程。后者是Binder通信的基础。
...
TLS(Thread Local Storage, 线程局部存储), 在Linux
