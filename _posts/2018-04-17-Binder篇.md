...
Service在启动过程中将自身信息注册到servicemanager中,因此servicemanager中维护了一个Service信息的列表。因此Client要使用服务时，只需向servicemanager提供所需Service的名字便可获取Service信息。servicemanager是由init启动的进程，本身也是一个Server。
```
